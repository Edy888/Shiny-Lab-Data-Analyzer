...
-   
    ...
    -   `kits` : информация из вкладки **НАБОРЫ** с коэффициентам
...
SQL_kits <- "INSERT IGNORE INTO kits_data_sql (id_lab, sn, Full_Test_Name, Reagent_Name, Kit_Lot, Bead_Lot,
...
kits_cols_short <- c(1,2,4,8,13,14,18,19,20,21, 33,34, 46,47,51:54, 62:69, 82:89, 102,105,108,109,134,137, 139:146, 164, 165)

kits <- openxlsx::read.xlsx('c:/Users/Eduard/Desktop/My_Projects/10. Shiny App/1. 4PL/data/kits/m6_1.xlsx', cols = kits_cols_short)
...
all_results <- openxlsx::read.xlsx('c:/Users/Eduard/Desktop/My_Projects/10. Shiny App/1. 4PL/data/results_lab/m6_N5521_1.xlsx', cols = col_results)
...
::: callout-note
### **Реактивные выражения в вашей среде**

1.  **`rv_results`**:

    -   **`rv_results$proceed_data`**: обработанные данные, отсортированные с правильными форматами и столбцами.

    -   **`rv_results$all_display_data`**: сводная статистика данных из `proceed_data`.

2.  **`filtered_data`**:

    -   Производит фильтрацию `rv_results$all_display_data` в зависимости от выбранных тестов, лабораторий и партий.

3.  **`slope_value` и `intercept_value`**:

    -   Используются для расчета наклона и смещения.

4.  **`calibrator_concentrations`**:

    -   Рассчитывает концентрации для лабораторных и MCD калибраторов.

5.  **`rv$calib_data`**:

    -   Хранит данные калибровки:

        -   `raw_data`: необработанные данные.

        -   `grouped_data`: сводная статистика.

6.  **`filtered_calib_data`**:

    -   Фильтрует данные калибровки в зависимости от выбранного теста и партий.

7.  **`rv_kits`**:

    -   Хранит данные для `Kits`, включая необработанные данные и их обновленное время.
:::
...
types <- readxl::read_xlsx("c:/Users/Eduard/Desktop/My_Projects/10. Shiny App/1. 4PL/data/4. Types/types_of_reactions.xlsx")

types <- types %>%
  filter(!is.na(Formula_Number)) |> 
...
save_to_sql <- function(data) {
  tryCatch({
    conn <- mysql_conn()  # Устанавливаем соединение
    
    # Формируем запрос для вставки данных
    values <- paste(
      apply(data, 1, function(row) {
        sprintf("('%s', %d, '%s')", 
                dbEscapeStrings(conn, row["Test"]),
                as.numeric(row["Formula_Number"]),
                dbEscapeStrings(conn, row["Type"]))
      }),
      collapse = ", "
    )
    
    query <- sprintf(
      "INSERT INTO type_of_reaction (Test, Formula_Number, Type) VALUES %s",
      values
    )
    
    dbExecute(conn, query)  # Выполняем запрос
    message("Data successfully saved to SQL!")
  }, error = function(e) {
    message("Error saving data to SQL: ", e$message)
  }, finally = {
    if (exists("conn") && !is.null(conn)) {
      dbDisconnect(conn)  # Закрываем соединение
    }
  })
}
